...
angular.module('dashboard')
    .controller('meetingDetailsCtrl', ['$log', '$rootScope', '$scope', '$state', 'CONST', 'StorageSrv', 'AttachmentData', 'ListData', 'PROPS', 'Utils', '$timeout', 'ENV', function ($log, $rootScope, $scope, $state, CONST, StorageSrv, AttachmentData, ListData, PROPS, Utils, $timeout, ENV) {
        ...
        function changeTopic(topic) {
            ...
            // var oldPm = self.pm;
            self.topicDelayOngoing = true;
            setData(topic);
            // self.pm check done because Win7IE11 white screen doesn't happen if switching between confidential topics so no delay then
            if ($rootScope.isIe /*&& !angular.equals(self.pm, self.pms.SECRET) && !angular.equals(oldPm, self.pms.SECRET)*/) {
                // Win7IE11 workaround: White ficker/stuck in white screen if
                // toggling public<==>confidential topics (dbPdf and imgViewer)
                self.topicDelayOngoing = true;
                ...
                self.topicTimeout = $timeout(function () {
                    ...
                    $timeout(function () { // WinIE11 white screen if change db-pdf and imgViewer simultaneously
                        self.topicDelayOngoing = false;
                        $log.debug("meetingDetailsCtrl.changeTopic: topicDelayOngoing timeout done");
                    }, 100);
                }, 100);
